...
public class IncomeTrackerTest {
    ...
    public void setUp() {
        ...
        testAmount = "100";
        testDescription = "Test income entry. Timestamp: "+ timestamp;
        testCategory = "Full Time Job";
    }
    ...
    public void testIncomeEntry() {
        ...
        // 2. Verify we've entered FinanceActivity
        ...
        // 5. Click on add income button
        onView(withId(R.id.income_add_btn)).perform(click());

        // 6. Verify we're on the add income screen
        onView(withId(R.id.entryAmount)).check(matches(isDisplayed())); // Entry amount field
        ...
        // 10. Check that the entry appears in the recent activities
        onView(withId(R.id.income_recent_activity)).check(matches(isDisplayed()));

        // 11. Navigate to full history to verify entry is there
        onView(withId(R.id.income_see_more)).perform(click());

        // 12. Verify we're on the income history screen
        onView(withText("Income History")).check(matches(isDisplayed()));
